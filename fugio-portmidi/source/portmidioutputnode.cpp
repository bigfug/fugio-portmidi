...
bool PortMidiOutputNode::initialise( void )
{
	...
	connect( mNode->context()->qobject(), SIGNAL(frameProcess(qint64)), this, SLOT(onFrameStart(qint64)) );
...
void PortMidiOutputNode::onFrameStart(qint64)
{
	if( !mDevice )
	{
		return;
...
QWidget *PortMidiOutputNode::gui()
{
	...
	if( ( GUI = new QComboBox() ) != nullptr )
	{
		...
		connect( GUI, &QComboBox::currentTextChanged, this, &PortMidiOutputNode::midiDeviceSelected );

		connect( this, &PortMidiOutputNode::midiDeviceChanged, GUI, &QComboBox::setCurrentText );
	}
